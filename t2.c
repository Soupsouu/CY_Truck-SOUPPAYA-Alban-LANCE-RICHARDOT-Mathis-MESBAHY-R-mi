...
void afficheAVL_K1(pAVL_N n, int *count) {
	if (n != NULL) {
	        afficheAVL_K1(n->fg, count);
        	printf("%s;%d;%d\n",n->Nom->nom,n->Nom->nbp,n->Nom->nbd);
		afficheAVL_K1(n->fd, count);
	}
}

pAVL_N AVL_K1(char * nom_f){
	FILE * fichier=NULL;
...
int main() {
    char *nom_f = "temps/tempstc.csv";
    int compteur = 10;
    pAVL_N arbre = AVL_K1(nom_f);
    afficheAVL_K1(arbre,&compteur);  
    libererAVL(arbre);
    return 0;
}
